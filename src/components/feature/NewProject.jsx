...
const NewProject = ({ alertType = true, onClose }) => {
	...
	useEffect(() => {
		if (loginUser) {
			...
			const defaultBrand = brandOptionsByCompany[defaultCompany]?.[0]?.value || '';
			setForm((prev) => ({
				...prev,
	...
	const handleSubmit = async () => {
		...
		if (form.title.length > 30) return openAlert('제목은 30자까지 입력할 수 있습니다.');
		if (!form.type) return openAlert('작업 종류를 선택하세요.');
		if (!form.description.trim()) return openAlert('프로젝트 내용을 입력하세요.');
		if (form.description.length > 500) return openAlert('내용은 500자까지 입력할 수 있습니다.');
					...
					<div className='alert_box'>
						...
						<div className='new-box'>
							...
							<div className='select-box'>
								<div>
									...
									{isStyleShipUser ? (
										...
										<Select
											name='company'
											value={form.company}
											options={companyOptions.filter((opt) => opt.value === loginUser?.userCompany)}
											onChange={() => {}}
											className='disabled'
										/>
									)}
								...
								<div>
									...
									<Input
										...
										setValue={(val) => setForm((prev) => ({ ...prev, writer: val }))}
										inputAble={false}
									/>
								</div>
							</div>
							...
							<div className='select-box'>
								<div className='_title'>
									...
									<Input
										...
										setValue={(val) => setForm((prev) => ({ ...prev, title: val }))}
							...
							<div className='select-box'>
								<div className='_type'>
									...
									<div>
										{['프로모션', '기획전', '이벤트', '외부몰', '오류 수정', '기능 수정/추가'].map((type, idx) => {
						...
						<div className='notice-box'>
							<ul>
								...
								<li>
									페이지 수정 및 변경을 요청시에는 첨부양식 사용바랍니다. <a href=''>양식다운받기</a>
								</li>
							</ul>
						</div>
